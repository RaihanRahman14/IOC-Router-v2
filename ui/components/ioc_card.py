"""Per-IOC expandable detail cards — all 11 provider tabs.

``render_ioc_cards`` only orchestrates: it resolves geolocation, decides which
tabs have data via the ``_x_has_data`` predicates, and dispatches into one
``_render_x_tab`` function per provider. Each of those is self-contained (it
reads only the arguments it's given) and independently testable — see
``tests/test_ioc_card.py``.
"""
...
def _render_confidence_score_card(row: dict) -> None:
    ...
    html = (
        ...
        f"padding:12px 14px;margin-bottom:12px;overflow:hidden;'>"
...
@dataclass
class _GeoContext:
    """Geolocation data resolved for one IOC — feeds the Shodan tab's geo panel."""
...
def _urlscan_screenshot_url(us: dict) -> str:
    """Return a URLScan result's screenshot URL, checking every field it can live in."""
    if not us:
        return ""
...
def _verdict_badge(verdict: str) -> str:
    """Build an HTML badge for a verdict string, defaulting to a neutral grey."""
    v = (verdict or "").lower()
    if v == "malicious":
...
def _virustotal_has_data(vt: dict) -> bool:
    """True when VT returned an object with any analysis data."""
    return bool(vt and (vt.get("stats") or vt.get("attributes") or vt.get("analysis_results")))


def _urlscan_has_data(us: dict) -> bool:
    """True when urlscan returned a scan record (queued, done, or scored)."""
    return bool(us and (us.get("uuid") or us.get("result") or us.get("page") or us.get("task")))


def _abuseipdb_has_data(ab: dict) -> bool:
    """True when AbuseIPDB returned a score or report count, error-free."""
    return bool(
        ab and not ab.get("error")
        and (ab.get("abuseConfidenceScore") is not None or ab.get("totalReports") is not None)
    )


def _threatfox_has_data(tf: dict) -> bool:
    """True when ThreatFox's query succeeded and returned at least one hit."""
    return bool(tf and tf.get("query_status") == "ok" and tf.get("data"))


def _malwarebazaar_has_data(mb: dict) -> bool:
    """True when MalwareBazaar's query succeeded and returned at least one sample."""
    return bool(mb and mb.get("query_status") == "ok" and mb.get("data"))


def _shodan_has_data(sh: dict) -> bool:
    """True for either Shodan mode: full summary, or InternetDB port/IP data."""
    return bool(sh and not sh.get("error") and (sh.get("summary") or sh.get("ports") or sh.get("queriedIp")))


def _dnsdumpster_has_data(dd: dict) -> bool:
    """True when DNSDumpster returned any DNS records, error-free."""
    return bool(
        dd and not dd.get("error")
        and (dd.get("soc_summary") or dd.get("dns_records") or dd.get("host_records"))
    )


def _hybrid_analysis_has_data(ha: dict) -> bool:
    """True when Hybrid Analysis returned an actual finding, not just a status message."""
    message = str(ha.get("message") or "").strip() if ha else ""
    return bool(
        ha
        and message not in {
...
def _mxtoolbox_has_data(mx: dict) -> bool:
    """True when MxToolBox ran at least one lookup, error-free."""
    return bool(mx and not mx.get("error") and mx.get("lookups"))


def _whoxy_has_data(wx: dict, ioc_type: str) -> bool:
    """True when Whoxy has WHOIS (domain/url) or reverse-WHOIS (keyword) data."""
    return bool(
        wx
        and not wx.get("error")
        and (
            (ioc_type in ("domain", "url") and wx.get("whois"))
            or (ioc_type == "whois" and wx.get("reverse_whois"))
        )
    )


def _ransomware_live_has_data(rl: dict) -> bool:
    """True when Ransomware.live found at least one victim listing."""
    return bool(rl and not rl.get("error") and rl.get("count", 0) > 0)


def _render_virustotal_tab(vt: dict, ioc_type: str) -> None:
    """Render the VirusTotal tab body."""
    def _fmt_ts(value):
        try:
            ts = int(value)
        except Exception:
...
def _render_urlscan_tab(us: dict) -> None:
    """Render the urlscan tab body."""
    page = us.get("page", {}) or {}
    task = us.get("task", {}) or {}
...
def _render_abuseipdb_tab(ab: dict) -> None:
    """Render the AbuseIPDB tab body."""
...
def _render_threatfox_tab(tf: dict, ioc_value: str) -> None:
    """Render the ThreatFox tab body."""
...
def _render_malwarebazaar_tab(mb: dict) -> None:
    """Render the MalwareBazaar tab body."""
...
def _render_shodan_tab(sh: dict, ab: dict, vt_attrs: dict, geo: _GeoContext) -> None:
    """Render the Shodan tab body, including the merged geolocation panel."""
    ...
    if geo.has_coords or vt_attrs.get("country") or ab.get("countryCode"):
        st.divider()
        ...
        _c_geo = geo.data.get("country")
        _cc_geo = geo.data.get("countryCode")
        _cc_ab = ab.get("countryCode")
        _c_vt = vt_attrs.get("country")
        ...
        _geo_row("City", geo.data.get("city"), "Geo")
        _geo_row("Region", f"{geo.data.get('regionName')} ({geo.data.get('region')})"
                 if geo.data.get("regionName") and geo.data.get("region")
                 else geo.data.get("regionName") or geo.data.get("region"),
                 "Geo")
        if geo.has_coords:
            _geo_row("Coordinates", f"{geo.lat}, {geo.lon}", "Geo")
        _geo_row("Postal Code", geo.data.get("zip"), "Geo")

        _nom_addr = geo.nom_data.get("address", {}) or {}
        ...
        _org_geo = geo.data.get("org")
        _isp_geo = geo.data.get("isp")
        _isp_ab = ab.get("isp")
        ...
        _vt_asn = vt_attrs.get("asn")
        _vt_owner = vt_attrs.get("as_owner")
        ...
        _geo_row("Continent", vt_attrs.get("continent"), "VirusTotal")
        _geo_row("RIR", vt_attrs.get("regional_internet_registry"), "VirusTotal")


def _render_dnsdumpster_tab(dd: dict) -> None:
    """Render the DNSDumpster tab body."""
...
def _render_hybrid_analysis_tab(ha: dict) -> None:
    """Render the Hybrid Analysis tab body."""
...
def _render_mxtoolbox_tab(mx: dict, ioc_value: str) -> None:
    """Render the MxToolBox tab body."""
    _mx_argument = mx.get("argument") or ioc_value
...
def _render_whoxy_tab(wx: dict, ioc_type: str, ioc_value: str) -> None:
    """Render the Whoxy tab body (WHOIS/reverse-WHOIS, both modes)."""
    _wx_rev = wx.get("reverse_whois") or {}
    _wx_label = wx.get("keyword") or wx.get("domain") or ioc_value
...
def _render_ransomware_live_tab(rl: dict, ioc_value: str) -> None:
    """Render the Ransomware.live tab body."""
    _rl_full = rl.get("full_domain") or ioc_value
...
def render_ioc_cards(run_results: dict) -> None:
    ...
    for ioc in items:
        ...
        with st.expander(
            ...
            _geo = _GeoContext(
                data=_geo_data, lat=_geo_lat, lon=_geo_lon,
                nom_data=_nom_data, has_coords=_has_coords,
            )
            ...
            _active_tabs = []
            if _virustotal_has_data(vt): _active_tabs.append("VirusTotal")
            if _urlscan_has_data(us): _active_tabs.append("urlscan")
            if _abuseipdb_has_data(ab): _active_tabs.append("AbuseIPDB")
            if _threatfox_has_data(tf): _active_tabs.append("ThreatFox")
            if _malwarebazaar_has_data(mb): _active_tabs.append("MalwareBazaar")
            if _shodan_has_data(sh): _active_tabs.append("Shodan")
            if _dnsdumpster_has_data(dd): _active_tabs.append("DNSDumpster")
            if _hybrid_analysis_has_data(ha): _active_tabs.append("Hybrid Analysis")
            if _mxtoolbox_has_data(mx): _active_tabs.append("MxToolBox")
            if _whoxy_has_data(wx, ioc.type): _active_tabs.append("Whoxy")
            if _ransomware_live_has_data(rl): _active_tabs.append("Ransomware Live")
            ...
            if "VirusTotal" in _ti:
                with _ti["VirusTotal"]:
                    _render_virustotal_tab(vt, ioc.type)
            if "urlscan" in _ti:
                with _ti["urlscan"]:
                    _render_urlscan_tab(us)
            if "AbuseIPDB" in _ti:
                with _ti["AbuseIPDB"]:
                    _render_abuseipdb_tab(ab)
            if "ThreatFox" in _ti:
                with _ti["ThreatFox"]:
                    _render_threatfox_tab(tf, ioc.value)
            if "MalwareBazaar" in _ti:
                with _ti["MalwareBazaar"]:
                    _render_malwarebazaar_tab(mb)
            if "Shodan" in _ti:
                with _ti["Shodan"]:
                    _render_shodan_tab(sh, ab, _vt_attrs, _geo)
            if "DNSDumpster" in _ti:
                with _ti["DNSDumpster"]:
                    _render_dnsdumpster_tab(dd)
            if "Hybrid Analysis" in _ti:
                with _ti["Hybrid Analysis"]:
                    _render_hybrid_analysis_tab(ha)
            if "MxToolBox" in _ti:
                with _ti["MxToolBox"]:
                    _render_mxtoolbox_tab(mx, ioc.value)
            if "Whoxy" in _ti:
                with _ti["Whoxy"]:
                    _render_whoxy_tab(wx, ioc.type, ioc.value)
            if "Ransomware Live" in _ti:
                with _ti["Ransomware Live"]:
                    _render_ransomware_live_tab(rl, ioc.value)
