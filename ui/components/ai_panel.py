...
``render_ai_panel`` only orchestrates widgets, session state, and the one
button-triggered action handler (``_run_ai_description_generation``, which
calls out to the AI provider). Everything it computes — prompts, the
evidence summary fed to ``analyzeThreat``, source links, the plain-text
share report — is a module-level function that takes ``run_results``
explicitly rather than closing over it, and is independently tested in
``tests/test_ai_panel.py``.
"""
...
def _clip(value: object, limit: int = 600) -> str:
    """Truncate value's str() form to limit chars, marking truncation."""
    text = str(value)
    if len(text) <= limit:
        return text
...
def _vt_url_id(url: str) -> str:
    """Return the VT URL-report id (base64url of the URL, no padding)."""
    raw_bytes = str(url or "").encode("utf-8")
    return base64.urlsafe_b64encode(raw_bytes).decode("utf-8").rstrip("=")


def _vt_gui_url(ioc_value: str, ioc_type: str, run_results: dict) -> str:
...
def _ha_text_payload(val: str, run_results: dict) -> object:
    """Return ha's dict, or 'No data' when empty or only a status message."""
...
def _provider_has_data(provider_name: str, ioc, run_results: dict) -> bool:
    """True when the named provider returned real data for this IOC."""
    vt_results = run_results["vt"]
...
def _build_ioc_links(selected_values: list[str], run_results: dict) -> str:
    """Build the 'Source: ... / - Provider: url' link block for selected IOCs."""
    ...
    for ioc in items:
        ...
        if ioc_type in {"ip", "domain", "url", "hash"} and _provider_has_data("virustotal", ioc, run_results):
            _vt_link = _vt_gui_url(value, ioc_type, run_results)
            if _vt_link:
                links.append(f"VirusTotal: {_vt_link}")

        if ioc_type in {"ip", "domain", "url", "hash"} and _provider_has_data("urlscan", ioc, run_results):
            if ioc_type == "ip":
        ...
        if ioc_type == "ip" and _provider_has_data("abuseipdb", ioc, run_results):
            links.append(f"AbuseIPDB: https://www.abuseipdb.com/check/{value}")

        if ioc_type in {"ip", "domain", "url", "hash"} and _provider_has_data("threatfox", ioc, run_results):
            links.append(f"ThreatFox: https://threatfox.abuse.ch/browse.php?search={quote_plus(value)}")

        if ioc_type == "hash" and _provider_has_data("malwarebazaar", ioc, run_results):
            links.append(f"MalwareBazaar: https://bazaar.abuse.ch/sample/{value}/")

        if ioc_type in {"ip", "domain"} and _provider_has_data("shodan", ioc, run_results):
            if ioc_type == "ip":
                links.append(f"Shodan: https://www.shodan.io/host/{value}")
            else:
                links.append(f"Shodan: https://www.shodan.io/domain/{value}")

        if ioc_type in {"domain", "url"} and _provider_has_data("dnsdumpster", ioc, run_results):
        ...
        if ioc_type in {"ip", "domain", "url", "hash"} and _provider_has_data("hybrid_analysis", ioc, run_results):
            if ioc_type == "hash":
...
def _build_prompt(
    selected_values: list[str], section: str, run_results: dict, tone: str, sanitize: bool,
) -> str:
    """Build the LLM prompt for the given section (SHORT / DESCRIPTION)."""
    items = run_results["items"]
    vt_results = run_results["vt"]
    urlscan_results = run_results["urlscan"]
    abuse_results = run_results["abuse"]
    ...
    for ioc in items:
        ...
        lines.append(f"  Hybrid Analysis: {_clip(_ha_text_payload(ioc.value, run_results))}")
...
def _obfuscate_domains_and_urls(text: str) -> str:
    """Defang every URL host and bare domain found in text."""
    raw = str(text or "")
...
def _build_analysis_summary(selected_values: list[str], run_results: dict) -> dict:
    """Aggregate provider + local-analyzer flags into analyzeThreat's evidence shape."""
    items = run_results["items"]
    vt_results = run_results["vt"]
    urlscan_results = run_results["urlscan"]
    abuse_results = run_results["abuse"]
...
def _to_bold_unicode(text: str) -> str:
    """Render text in Unicode mathematical bold (no markdown ** needed)."""
    out = []
    for ch in str(text):
        if "A" <= ch <= "Z":
...
def _derive_threat_category(ev: dict) -> str:
    """Pick a one-line threat category label from the evidence dict."""
...
def _derive_attack_status(ev: dict) -> str:
    """Classify overall attack status (Active / Prevented/Blocked / Attempted / none)."""
    has_active = any(
        [
...
def _build_reason_fallbacks(summary: dict, state: str, level: str) -> list[str]:
    """Build the 3 fallback reason lines shown when the AI omitted its own."""
...
def _format_threat_text_for_box(raw_text: str, summary: dict) -> str:
    """Parse a raw AI threat-analysis reply into the fixed display block."""
...
def _build_share_text(selected_values: list[str], run_results: dict) -> str:
    """Build the full plain-text 'Shareable Text' report for selected IOCs."""
    items = run_results["items"]
    vt_results = run_results["vt"]
    abuse_results = run_results["abuse"]
    shodan_results = run_results["shodan"]
    ...
    # ── Threat Analysis ───────────────────────────────────────────────
    _ta_sum    = _build_analysis_summary(selected_values, run_results)
    ...
    # ── Sources ───────────────────────────────────────────────────────
    _links_text = _build_ioc_links(selected_values, run_results)
    if _links_text:
...
def _text_with_copy(label: str, text: str, height: int, key: str) -> None:
    """Render a text_area plus a JS 'Copy' button that copies its contents."""
...
def _flag_source_url(source: str, ioc_value: str, ioc_type: str, run_results: dict) -> str:
    """Return the direct Threat Intel URL for a given flag source + IOC."""
    dnsd_results = run_results.get("dnsd", {})
    s = source.lower()
    if "virustotal" in s or s == "vt":
        _vt_link = _vt_gui_url(ioc_value, ioc_type, run_results)
        if _vt_link:
            return _vt_link
    if "urlscan" in s:
        if ioc_type == "ip":
...
def render_ai_panel(run_results: dict, settings) -> None:
    ...
    def _run_ai_description_generation() -> bool:
        ...
        desc_prompt = _build_prompt(selected, "DESCRIPTION", run_results, tone, sanitize)
        ...
        st.session_state["ai_ioc_links"] = _build_ioc_links(selected, run_results)
        return True
    ...
    if desc_text or selected:
        ...
        _ta_summary = _build_analysis_summary(selected or [], run_results)
        ...
        with st.expander("**Threat Analysis**", expanded=False):
                ...
                for _sev in ("CRITICAL", "HIGH", "MEDIUM", "LOW", "INFO"):
                    ...
                    with st.expander(f"{_se} {_sev_label} — {len(_grp)} indicator(s)"):
                        for _f in _grp:
                            ...
                            _f_src_url = (
                                _f.get("source_url")
                                or _flag_source_url(_f["source"], _f_ioc_val, _f_ioc_type, run_results)
                            )
                            _src_badge = (
            ...
            _ioc_links_text = st.session_state.get("ai_ioc_links") or _build_ioc_links(selected or [], run_results)
            if _ioc_links_text:
                st.divider()
        ...
        st.session_state["share_text"] = _build_share_text([ioc.value for ioc in items], run_results)
